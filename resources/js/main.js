...
try {
    ...
    function calculate_actual_population() {
        ...
        if (actual_population > city_vals.population_max)
            actual_population = city_vals.population_max;
    ...
    function create2darray(rows, cols, fill) {
        let array = Array.from({ length: rows }, () => Array.from({ length: cols }, () => (Object.assign({}, fill))));
        return array;
    }
    const world = {
        rows: 50,
    ...
    function generateLakes(chance = 0.005, growthChance = 0.2, iterations = 3) {
        try {
            ...
            for (let y = 0; y < world.rows; y++) {
                for (let x = 0; x < world.cols; x++) {
                    if (Math.random() < chance) {
                        world.grid[y][x] = Object.assign({}, tile_types.water);
                    }
                }
            }
            ...
            for (let i = 0; i < iterations; i++) {
                const newGrid = world.grid.map((row, y) => row.map((cell, x) => {
                    ...
                    if (cell.id === tile_types.grass.id && neighbors > 0) {
                        ...
                        if (Math.random() < growProbability) {
                            return Object.assign({}, tile_types.water);
                        }
                    }
                    return cell;
                }));
        ...
        try {
            ...
            for (let y = 0; y < world.rows; y++) {
                for (let x = 0; x < world.cols; x++) {
                    ...
                    city_vals.jobs += world.grid[y][x].tile_data.jobs;
                    city_vals.power -= world.grid[y][x].tile_data.power_taken;
                    city_vals.power += world.grid[y][x].tile_data.power_provided;
                    city_vals.water -= world.grid[y][x].tile_data.water_taken;
                    city_vals.water += world.grid[y][x].tile_data.water_provided;
                    ...
                    */
                    ...
                    //Draw
                    //Texture Test
                    if (texturesAtlas[world.grid[y][x].tile_data.name] && world.grid[y][x].id !== 1) {
                        let texture = new Image();
                        texture.src = texturesAtlas[world.grid[y][x].tile_data.name];
                        ctx.drawImage(texture, (x + world.offsetX) * tileSize, (y + world.offsetY) * tileSize, tileSize, tileSize);
                    }
                    else if (world.grid[y][x].tile_data.name === 'road') {
                        road(x, y);
                        let texture = new Image();
                    ...
                    tile_counts[world.grid[y][x].tile_data.name] += 1;
                    //if(city_vals.power <= 0) {
            ...
            if (city_vals.city_policies.ads || city_vals.city_policies.water_lessenation || city_vals.city_policies.energy_lessenation) {
                ...
                document.getElementById('expense').textContent = city_vals.maintainence.toString();
            }
            if (city_vals.city_policies.water_lessenation) {
                city_vals.water += 4;
            }
            ...
            document.getElementById('city-pop').textContent = city_vals.population_max.toString();
            document.getElementById('city-actual-population').textContent = city_vals.population.toString();
            document.getElementById('city-income').textContent = city_vals.tax.toString();
            document.getElementById('expense').textContent = city_vals.maintainence.toString();
            document.getElementById('city-jobs').textContent = city_vals.jobs.toString();
            document.getElementById('city-power').textContent = city_vals.power.toString();
            document.getElementById('city-money').textContent = city_vals.money.toString();
            document.getElementById('city-happiness').textContent = city_vals.happiness.toString();
            document.getElementById('city-water').textContent = city_vals.water.toString();
            document.getElementById('city-demand').textContent = city_vals.demand.toString();
            document.getElementById('citizen happiness display').textContent = habitabitabality_info_String().toString();
            document.getElementById('city-points').textContent = city_vals.points.toString();
    ...
    canvas.addEventListener('click', (e) => {
        try {
            ...
            if (mode == 'road' && road_mode == 'easy') {
                ...
                if (mode === 'demolish') {
                    city_vals.money += world.grid[gridY][gridX].tile_data.price;
                    if (world.grid[gridY][gridX].previous_state == null || undefined) {
                        world.grid[gridY][gridX] = Object.assign({}, tile_types.grass);
                    }
                    else {
                ...
                if (tile_types[mode].unlocked && mode !== 'demolish') {
                    ...
                    if (mode.includes('water_collecter')) {
                        if (isAdjacentto(gridX, gridY, 1) && isAdjacentto(gridX, gridY, 4)) {
                            city_vals.money -= tile_types[mode].tile_data.price;
                            world.grid[gridY][gridX] = Object.assign({}, tile_types[mode]);
                    ...
                    if (mode !== 'demolish' && !mode.includes('water_collecter') && tile_types[mode].tile_data.requiresRoad) {
                        if (isAdjacentto(gridX, gridY, 1)) {
                            world.grid[gridY][gridX] = Object.assign({}, tile_types[mode]);
                            city_vals.money -= tile_types[mode].tile_data.price;
                        }
                        else
                            new notif('Must be placed next to road');
                    }
    ...
    setInterval(() => {
        if (!paused) {
            ...
            document.getElementById('city-money').textContent = city_vals.money.toString();
            document.getElementById('time').textContent = dayTime.toString();
        }
    }, 1500);
    document.addEventListener('keydown', function (event) {
        ...
        if (event.key === 'M')
            city_vals.money += parseInt(prompt('?+int?: '));
        if (event.key === 'P')
            city_vals.points += parseInt(prompt('?+int?: '));
    ...
    document.getElementById('submit_input').addEventListener('click', () => {
        //new notif('hai')
        loading = true
        const file = (document.getElementById('cty_input') as FileSele).files
