...
const canvas: HTMLCanvasElement = document.getElementById('world') as HTMLCanvasElement;
        ...
        if(mode == 'road' && road_mode == 'easy') {
            ...
            if(tile_types[mode].unlocked && mode !== 'demolish') {
                ...
                if(mode !== 'demolish' && !mode.includes('water_collecter') && tile_types[mode].tile_data.requiresRoad) {
                    if(isAdjacentto(gridX, gridY, 1)) {world.grid[gridY][gridX] = {...tile_types[mode]}; city_vals.money -= tile_types[mode].tile_data.price
